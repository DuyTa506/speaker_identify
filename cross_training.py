...
def main():
    model_path = 'saved_models_cross_entropy/6/'
    use_cuda = False
    ...
    train_dataset = FBanksCrossEntropyDataset('dataset-speaker-csf/fbanks-train')
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, **kwargs)

    test_dataset = FBanksCrossEntropyDataset('dataset-speaker-csf/fbanks-test')
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=True, **kwargs)

    model = FBankCrossEntropyNetV2(num_layers=6, reduction='mean').to(device)
